...
const char PREFIX[] = "{\"config\":{\"encoding\":\"LINEAR16\",\"sampleRateHertz\":8000,\"languageCode\": \"en-US\",\"speechContexts\": [{\"phrases\": [\"draw\"]}]}, \"audio\": {\"content\":\"";
const char SUFFIX[] = "\"}}"; //suffix to POST request
const int AUDIO_IN = A2; //pin where microphone is connected
const char API_KEY[] = "AIzaSyA2tMkCBlkiQL-MtK-8BTVXKQo1Ip2fBCw"; //don't change this
...
void loop() {
  float MIC_READING = analogRead(AUDIO_IN);
...
void record_audio() {
  ...
  while (sample_num < NUM_SAMPLES) { //read in NUM_SAMPLES worth of audio data
    ...
    raw_samples[sample_num % 3] = value; //remove 1.0V offset (from 12 bit reading)
    sample_num++;
    if (sample_num % 3 == 0) {
